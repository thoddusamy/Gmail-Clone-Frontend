...
const SendMails = () => {
    ...
    document.title = `Sent Mail - Gmail Clone`
    ...
    dispatch(inputvalue({ inputValue: 'in:sent' }))
    dispatch(setcollectionname({ collectionName: "send_mails" }))
    ...
    useEffect(() => {
        let SendMails = async () => {
            try {
                dispatch(loadingtext({ isLoading: true }))
                let fetchSendMails = await axios.get(`${config.api}/main/sendMail`)
                await context.setSendMails(fetchSendMails.data)
                dispatch(loadingtext({ isLoading: false }))
            } catch (error) {
                console.log(error);
            }
    ...
    const filteredMails = context.sendMails.filter((fillMail) =>
    ...
    return (
        <div className='sendmails__container' style={quickSetting.value ? { width: "912px" } : { width: "80%" }}>
            <div className="sendSticky_top">
                ...
                <div className="bottom_icons">
                    <div className="icons_section">
                        <div className="iconSection_left">
                            ...
                            <div className={showMorePopup ? "morePopupSend moreopts" : "morePopupSend lessopts"}>
                        ...
                        <div className="iconSection_right">
                            ...
                            <div className={showNewestOldestPopup ? "newestOlder_popupSend moreopts" : "newestOlder_popupSend lessopts"}>
                            ...
                            <div className={showSplitPanePopup ? "splitPanemode_popupSend moreopts" : "splitPanemode_popupSend lessopts"}>
                                <p>No split</p>
                                <p>Vertical split</p>
            ...
            <div className="mailCard_container">
                {
                    filteredMails.map((sendMail) => {
                        return <MailCard sendMail={sendMail} key={sendMail._id} />
                    })
                }
            </div>
        </div>
