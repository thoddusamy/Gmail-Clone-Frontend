...
const StarredMails = () => {
    ...
    document.title = `Starred - Gmail Clone`

    const context = useContext(ContextApi)

    const dispatch = useDispatch()
    dispatch(mailcount({ count: context.dynamicMails.length }))
    dispatch(inputvalue({ inputValue: 'is:starred' }))
    dispatch(setcollectionname({ collectionName: "starred_mails" }))
    ...
    useEffect(() => {
        let Starred = async () => {
            try {
                dispatch(loadingtext({ isLoading: true }))
                let fetchStarredMails = await axios.get(`${config.api}/main/starred`)
                context.setStarredMails(fetchStarredMails.data)
                dispatch(loadingtext({ isLoading: false }))
            } catch (error) {
                console.log(error);
            }
    ...
    const filteredMails = context.starredMails.filter((fillMail) =>
    ...
    return (
        <div className='starred__container' style={quickSetting.value ? { width: "912px" } : { width: "80%" }}>
            <div className="starredSticky_top">
                ...
                <div className="bottom_icons">
                    <div className="icons_section">
                        <div className="iconSection_left">
                            ...
                            <div className={showMorePopup ? "morePopupStar moreopts" : "morePopupStar lessopts"}>
                        ...
                        <div className="iconSection_right">
                            ...
                            <div className={showNewestOldestPopup ? "newestOlder_popupStar moreopts" : "newestOlder_popupStar lessopts"}>
                            ...
                            <div className={showSplitPanePopup ? "splitPanemode_popupStar moreopts" : "splitPanemode_popupStar lessopts"}>
                                <p>No split</p>
                                <p>Vertical split</p>
            ...
            <div className="mailCard_container">
                {
                    filteredMails.map((starredMail) => {
                        return <MailCard starredMail={starredMail} key={starredMail._id} />
                    })
                }
            </div>
        </div>
